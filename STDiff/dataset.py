...
class ImageNetVidDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, image_size=256, batch_size=2, len_seq=8, path="", path_weather="", path_scaler="", phase="train",
        ...
        for fol in os.listdir(path):
            ...
            images, temps, dates, weather = self.generate_many2many_data2(self.l_seq, images, temps, dates, weather)
            if images.shape[0] > 0:
        ...
        self.wnormalizer = StandardScaler()
        weather = self.wnormalizer.fit_transform(
            self.weather.reshape(self.weather.shape[0] * self.weather.shape[1], -1))
    ...
    def load_data(self, path, fol):
        imgfile = pd.read_csv(path + fol + '/images.csv',
                              dtype={'station_name': str, 'station_id': int, 'image_id': int, 'timestamp': str,
                                     'filename': str, 'url': str}, parse_dates=['timestamp'])
        valuesfile = pd.read_csv(path + fol + '/values.csv',
                                 dtype={'station_name': str, 'station_id': int, 'dataset_id': int, 'series_id': int,
                                        'variable_id': str, 'timestamp': str, 'value': float},
        ...
        weatherfile = []
        for w in all_files:
            file = pd.read_excel(os.path.join(self.path_weather, "Weather", w), skiprows=[0, 1, 3],
            ...
            file[["Time of Daily Temp Max", "Time of Min. Temp", "Time of Max Wind Spd", "Time of Min. Wind Spd."]] = \
            file[["Time of Daily Temp Max", "Time of Min. Temp", "Time of Max Wind Spd",
                  "Time of Min. Wind Spd."]].applymap(datetime.timestamp)
